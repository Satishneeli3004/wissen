layui.define(['layer', 'config', 'layRouter'], function (exports) {
    ...
    var admin = {
        ...
        refreshToken: function (callback, errorCallback) {
            debugger
            var obj = config.getToken();
            if (obj && obj.refresh_token) {
                $.ajax({
                    url: config.base_server + "/oauth/token",
                    data: {grant_type: "refresh_token", "refresh_token": obj.refresh_token},
                    type: "post",
                    async: false,
                    dataType: "json",
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res.access_token) {
                            res.timestamp = new Date().getTime() + (res.expires_in * 1000);
                            config.putToken(res);
        ...
        req: function (url, data, success, method, error) {
            method = method || "get";
            admin.ajax({
        ...
        ajax: function (param) {
            ...
            param.success = function (result, status, xhr) {
                ...
                if (errorCallback || jsonRs.code == 0) {
                    successCallback(result, status, xhr);
                    return;
                }
                if (jsonRs.code != 0) {
                    layer.closeAll('loading');
                    layer.msg(jsonRs.msg || jsonRs.error, {icon: 2});
                }
            };
            param.error = function (xhr) {
                ...
                if (errorCallback) {
                    if (json) {
                        param.success({code: xhr.status, msg: json.msg || json.error});
                    } else {
                    ...
                    layer.closeAll('loading');
                    if (json) {
                        layer.msg(json.msg || json.error, {icon: 2});
                    } else {
                        var statusCode = {401: "用户未登录", 403: "用户权限不足", 500: "系统内部错误", 0: "网络连接异常"};
                        layer.msg(statusCode[xhr.status] || "未知异常", {icon: 2});
                    }
                }
            };
            param.beforeSend = function (xhr) {
                var headers = config.getAjaxHeaders(param.url);
